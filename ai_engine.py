...
def vote_parser(ai_response):
    ...
    if "entry" in ai_response.lower() and "sl" in ai_response.lower(
    ) and "tp" in ai_response.lower():
        vote = "YES"
...
def is_duplicate_signal(new_signal):
    try:
        with open("last_signal.json", "r") as f:
            last = json.load(f)
            return (last.get("entry") == new_signal.get("entry")
                    and last.get("sl") == new_signal.get("sl")
                    and last.get("tp") == new_signal.get("tp")
                    and last.get("direction") == new_signal.get("direction"))
    except:
        return False
...
def main():
    for tf in TIMEFRAMES:
        ...
        print(f"📊 Live BTCUSDT ({tf}) Price: {candles[-1]['close']}")
        if detect_trade(candles, tf):
            ...
            if yes_votes >= 2:
                ...
                result = {
                    ...
                    tf,
                    "direction":
                    "BUY" if "buy" in chatgpt_resp.lower()
                    or "buy" in grok_resp.lower()
                    or "buy" in deepshik_resp.lower() else "SELL",
                    "votes":
                    f"{yes_votes} YES / {3 - yes_votes} NO",
                    "chatgpt":
                    chatgpt_detail,
                    "grok":
                    grok_detail,
                ...
                result["outcome"] = evaluate_trade_outcome(
                    result, future_candles)
