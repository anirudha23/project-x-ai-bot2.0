...
def detect_trade(candles, tf):
    ...
    # --- Trend Detection (Simple HH/HL or LL/LH check) ---
    bullish_trend = candles[-5]["high"] < candles[-4]["high"] < candles[-3]["high"]
    bearish_trend = candles[-5]["low"] > candles[-4]["low"] > candles[-3]["low"]

    # --- CHoCH Check ---
    choch_up = candles[-3]["low"] > candles[-2]["low"] < candles[-1]["low"]
    choch_down = candles[-3]["high"] < candles[-2]["high"] > candles[-1]["high"]
    ...
    sweep_low = candles[-2]["low"] < min(c["low"] for c in candles[-6:-2])
    sweep_high = candles[-2]["high"] > max(c["high"] for c in candles[-6:-2])
    ...
    # --- Final Confirmation ---
    if (bullish_trend and choch_up and sweep_low and bos_up and valid_ob) or \
       (bearish_trend and choch_down and sweep_high and bos_down and valid_ob):
        return True
